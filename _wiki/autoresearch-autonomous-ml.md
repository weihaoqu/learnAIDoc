...
related: ["Claude Code /loop: Scheduled Prompts on Autopilot", "Claude Code 101 — Anthropic's Official Onboarding Course", "Karpathy: The End of Coding — Agents, AutoResearch, and the Loopy Era", "alphaXiv MCP — Semantic ArXiv Search Directly in Claude Code", "AI-Assisted Research Workflow: Formulate → Find → Judge → Verify → Execute → Monitor → Record", "Awesome-Auto-Research-Tools — A Curated Map of Automated-Science Projects", "Beyond /goal — The Orchestrator + Headless Pattern for Long-Running Claude Sessions"]
icon: "🔬"
